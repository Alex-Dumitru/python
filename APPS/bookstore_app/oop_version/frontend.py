...
copy-pasted from course. Does not work. Do it from scratch from the other one (functional)
"""


import tkinter as tk
from backend import Database as Db  # custom module to import the backtk.END functionality of the app
...
class Window(object):
    ...
    def __init__(self, window):
        ...
        self.list1.bind('<<tk.ListboxSelect>>', self.get_selected_row)

        b1 = tk.Button(window, text="View all", width=12, command=self.view_command)
        b1.grid(row=2, column=3)

        b2 = tk.Button(window, text="Search entry", width=12, command=self.search_command)
        b2.grid(row=3, column=3)

        b3 = tk.Button(window, text="Add entry", width=12, command=self.add_command)
        b3.grid(row=4, column=3)

        b4 = tk.Button(window, text="Update selected", width=12, command=self.update_command)
        b4.grid(row=5, column=3)

        b5 = tk.Button(window, text="Delete selected", width=12, command=self.delete_command)
    ...
    def search_command(self):
        self.list1.delete(0, tk.END)
        for row in db.search(self.title_text.get(), self.author_text.get(), self.year_text.get(),
                                   self.isbn_text.get()):
            self.list1.insert(tk.END, row)
    ...
    def update_command(self):
        db.update(self.selected_tuple[0], self.title_text.get(), self.author_text.get(), self.year_text.get(),
                        self.isbn_text.get())
